...
// ★★★ 変更点: "normalSettings" の行を削除 ★★★
// (sharedSettings は常に表示なので、JSで参照不要)
...
const KOTO_TUNINGS = {
    'hirajoshi': {
        name: '平調子',
        intervals: [0, -7, -5, -4, 0, 1, 5, 7, 8, 12, 13, 17, 19],
...
/**
 * ★★★ 変更点: updateMode 関数 ★★★
 * "normalSettings" の表示/非表示の行を削除
 */
function updateMode(mode) {
    currentMode = mode;
    if (mode === 'normal') {
        // normalSettings.style.display = 'flex'; // 削除
        ...
        labelModeKoto.style.color = '#abb2bf'; // 修正済み
        ...
        // normalSettings.style.display = 'none'; // 削除
...
function detectPitch() {
    // (変更なし、Float3DArray は Float32Array に修正済み)
...
function updateUI(noteInfo, detune) {
    ...
    if (noteInfo && Math.abs(detune) < 5) {
        meterNeedle.style.backgroundColor = "#98c379";
    } else {
        meterNeedle.style.backgroundColor = "#e06c75";
    }
}
